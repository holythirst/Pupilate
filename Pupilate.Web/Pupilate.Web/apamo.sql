...
Dreamweaver $485 ==> Bought by Olamide
...
CREATE TABLE WonderCard
(
	cPin VARCHAR(20) NULL,
	cType VARCHAR(10) NULL, /*Green or Red*/
    cAmount DOUBLE NULL,
    cStatus VARCHAR(10) NULL, /*Used or Unused*/
    cExpiryDate VARCHAR(10) NULL,
	PRIMARY KEY (cPin)
);
...
(
    ...
    cSchoolCode VARCHAR(20) NOT NULL,
	dExpiryDate DATE NULL,
	cSecret VARCHAR(100) NULL,
	cAnswer VARCHAR(20) NULL,
    PRIMARY KEY (cID)
);
...
(
	...
	PRIMARY KEY (cFName),
    UNIQUE INDEX reg_ses_UNIQUE (cID ASC),
    CONSTRAINT FK_reg_ses_cSSRNo FOREIGN KEY (cSSRNo) REFERENCES reg_sch (cSSRNo)
);
Insert Into Ses(cSSRNo, dStart , dEnd , cFName) values ('', '2014/09/09','2015/09/07', '2014/2015');
...
/* The cSID is gotten fron the state code/session/admission number @ school*/
...
JOIN reg_sch b ON a.cSSRNo = b.cSSRNo
JOIN Class_GTS c ON a.cSID = c.cSID
JOIN Class_ST d ON d.cSSRNo = b.cSSRNo
 WHERE c.AutoNo = '001'
)
...
(
	...
	SOrigin VARCHAR(20) NULL,
	HAddress VARCHAR(100) NULL,
	Phone VARCHAR(30) NULL,
	MTongue VARCHAR(20) NULL,
	Dis VARCHAR(30) NULL,
	PAI VARCHAR(50) NULL,
	...
	PGPhone VARCHAR(30) NULL,
	PGAdd VARCHAR(100) NULL,
	Pix LONGBLOB NULL,
...
(
	...
	cSub VARCHAR(30) NULL,
	cTID VARCHAR(15) NULL,
	cSession VARCHAR(10) NULL,
	cTerm VARCHAR(10) NULL,
	CONSTRAINT PK_SubTeacher PRIMARY KEY (cID),
	CONSTRAINT FK_SubTeacher2 FOREIGN KEY (cTID) References reg_tut (cTID)
);
...
(
	cID INT NOT NULL AUTO_INCREMENT,
	cSSRNo VARCHAR(20) NOT NULL,
    cSchool VARCHAR(12) NOT NULL, /*Kindagerten, Nursery, Basic, Primary, Junior, Seor, etc*/
	cName INT NOT NULL, /*1-9*/
	cWing VARCHAR(2) NOT NULL, /*A-Z*/
	cDep VARCHAR(15) NOT NULL, /*Science, Arts, etc.*/
	cFName VARCHAR(10) NOT NULL,
    UNIQUE INDEX cFName_UNIQUE (cFName ASC),
	PRIMARY KEY (cID)
);

CREATE TABLE reg_sch_ses
(
	cID INT NOT NULL AUTO_INCREMENT,
	...
	cFName VARCHAR(10) NOT NULL,
	PRIMARY KEY (cID),
    CONSTRAINT FK_reg_sch_ses_cFName FOREIGN KEY (cFName) REFERENCES reg_ses (cFName)
);
Insert Into Ses(cSSRNo, dStart , dEnd , cFName) values ('', '2014/09/09','2015/09/07', '2014/2015');
...
(
	...
	CONSTRAINT FK_StaAward FOREIGN KEY (cEID) References StaAppoINT (cEID)
);

CREATE TABLE StaBlack
(
	cID INT NOT NULL AUTO_INCREMENT,
	cEID VARCHAR(15) NULL,
	dDATE DATE NULL,
	...
	CONSTRAINT FK_StaBlack FOREIGN KEY (cEID) References StaAppoINT (cEID)
);
...
(
	...
	CONSTRAINT FK_StaAtt FOREIGN KEY (cEID) References StaAppoINT (cEID)
);
...
(
	...
	CONSTRAINT FK_StaPaySlip FOREIGN KEY (cEID) References StaAppoINT (cEID),
...
(
	...
	CONSTRAINT FK_StaLeave FOREIGN KEY (cEID) References StaAppoINT (cEID)
);
...
(
	...
	CONSTRAINT FK_StuAtt2 FOREIGN KEY (cEID) References StuAdmin (cEID)
);
...
(
	...
	CONSTRAINT FK_StuAward2 FOREIGN KEY (cEID) References StuAdmin (cEID)
);
...
(
	...
	CONSTRAINT FK_StuBlack2 FOREIGN KEY (cEID) References StuAdmin (cEID)
);

CREATE TABLE StuStaCol
(
	cID INT NOT NULL AUTO_INCREMENT,
	cEID VARCHAR(15) NULL,
	cSession VARCHAR(10) NULL,
	...
	CONSTRAINT FK_StuStaCol2 FOREIGN KEY (cEID) References StuAdmin (cEID)
);
...
CREATE TABLE ChatLogDetails
(
	cID INT NOT NULL AUTO_INCREMENT,
	cName VARCHAR(20) NULL,
	dDATE DATE NULL,
	tTime DATE NULL,
	cDes VARCHAR(20) NULL,
    PRIMARY KEY (cID)
);
...
CREATE TABLE ChatUserBlog
(
	cID INT NOT NULL AUTO_INCREMENT,
	cName VARCHAR(20) NULL,
	dDATE DATE NULL,
	tTime DATE NULL,
	cMsg VARCHAR(500) NULL,
	cDes VARCHAR(20) NULL,
    PRIMARY KEY (cID)
);
...
(
	cID INT NOT NULL AUTO_INCREMENT,
	cSender VARCHAR(20) NULL,
	cReceiver VARCHAR(20) NULL,
	cLoop VARCHAR(20) NULL,
	cMsg VARCHAR(300) NULL,
	dDATE DATE NULL,
	tTime DATE NULL,
	cDes VARCHAR(20) NULL,
	cShow VARCHAR(20) NULL,
    PRIMARY KEY (cID)
);

CREATE TABLE ChatMailCurrent
(
	cID INT NOT NULL AUTO_INCREMENT,
	cName VARCHAR(20) NULL,
	cDes VARCHAR(20) NULL,
...
(
	...
	cBankBranch VARCHAR(50) NOT NULL,
	cAcctNo VARCHAR(30) NOT NULL,
	cAddress VARCHAR(120) NULL,
	cPhone VARCHAR(30) NULL,
	cOpBal DOUBLE NOT NULL,
	cCurBal DOUBLE NULL,
	dOpDATE DATE NOT NULL,
	cLastDep DOUBLE NULL,
	cLastDepDATE DATE NULL,
	cLastWith DOUBLE NULL,
	cLastWithDATE DATE NULL,
	UNIQUE INDEX cBankDetails_UNIQUE (cAcctNo ASC),
    PRIMARY KEY (cID)
);
...
(
	cID INT NOT NULL AUTO_INCREMENT,
	cAcctID VARCHAR(30) NULL,
	cSlipNo VARCHAR(20) NULL,
	cAmount DOUBLE NULL,
	dDATE DATE NULL,
	cDesc VARCHAR(100) NULL,
	cEmpID VARCHAR(50) NULL,
	CONSTRAINT PK_BankDep PRIMARY KEY (cID),
	CONSTRAINT FK_BankDep FOREIGN KEY (cAcctID) References BankDetails(cAcctNo)
);
...
(
	...
	dDATE DATE NULL,
	cDesc VARCHAR(100) NULL,
	cEmpID VARCHAR(15) NULL,
	CONSTRAINT PK_BankWith PRIMARY KEY (cID),
	CONSTRAINT FK_BankWith FOREIGN KEY (cAcctID) References BankDetails(cAcctNo)
);

CREATE VIEW FeesPaymentCurDef as 
(
	SELECT DISTINCT FeesPaymentCur.cBID, FeesPaymentCur.mTotal, FeesPaymentCur.mAmount, FeesPaymentCur.mBal, FeesBilling.cEID, FeesBilling.cSession, FeesBilling.cTerm, FeesBilling.cClass
...
(
Select Prefects.cID as "ID", Prefects.cEID as "Student ID", Prefects.cSession as "Session", Prefects.cPost as "Post", StuAdmin.SName + " " + StuAdmin.FName + " " + StuAdmin.OName as "Full Name"
from Prefects INNER JOIN StuAdmin ON Prefects.cEID =StuAdmin.cEID
);